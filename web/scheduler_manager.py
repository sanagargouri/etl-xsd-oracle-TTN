...
def run_etl_job(xml_folder, dossier_traites, dossier_erreurs,
    ...
    ouvre une connexion Oracle, synchronise chaque schéma configuré dans
    SCHEMA_TYPE_CONFIG avec LIASSE.DOSSIER (CODE_TYPE_DOSSIER, ACTIVITE et
    période de dates lus directement en base, configurables depuis le
    dashboard -- plus de liste fixe TITRE/TCE), traite le dossier
    a_traiter (chaque fichier utilise le schéma choisi manuellement au
    dépôt, via la file d'attente DDL_XSD_PENDING_FILES), ferme la connexion.
...
def trigger_now(xml_folder, dossier_traites, dossier_erreurs,
    ...
    remplacement. Utilise la même configuration (SCHEMA_TYPE_CONFIG) que
